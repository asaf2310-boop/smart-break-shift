...
function addCanvasPagesToPdf(doc, canvas, startOnNewPage = false) {
...
function buildPhoneSystemStepHtml(step, index) {
  const tip = step.tip
    ? `<p style="margin:10px 0 0;padding:9px 12px;background:#fffbeb;border:1px solid #fde68a;border-radius:8px;font-size:12px;color:#92400e !important;-webkit-text-fill-color:#92400e;">טיפ: ${escapeHtml(step.tip)}</p>`
    : "";
  const images = (step.images || [])
    .map(
      (image) => `
        <figure style="margin:14px 0 0;border:1px solid #e5e7eb;border-radius:10px;overflow:hidden;background:#ffffff;break-inside:avoid;">
          <div style="padding:10px;text-align:center;">
            <img
              src="${escapeHtml(image.url)}"
              alt="${escapeHtml(image.alt)}"
              crossorigin="anonymous"
              style="display:inline-block;max-width:100%;width:auto;height:auto;max-height:620px;object-fit:contain;"
            />
          </div>
          <figcaption style="padding:7px 10px;border-top:1px solid #e5e7eb;font-size:11px;color:#6b7280 !important;-webkit-text-fill-color:#6b7280;text-align:right;">
  ...
  return `
    <article style="margin:0 0 16px;padding:14px;border:1px solid #e5e7eb;border-radius:12px;background:#ffffff;">
      <div style="display:flex;align-items:flex-start;gap:10px;">
        <span style="display:inline-flex;align-items:center;justify-content:center;min-width:26px;height:26px;background:#eef2ff;color:#4f46e5 !important;-webkit-text-fill-color:#4f46e5;border-radius:7px;font-size:12px;font-weight:700;">${index + 1}</span>
        <div>
          <h3 style="margin:0 0 5px;font-size:14px;color:#111827 !important;-webkit-text-fill-color:#111827;">${escapeHtml(step.name)}</h3>
          <p style="margin:0;font-size:12px;line-height:1.7;color:#374151 !important;-webkit-text-fill-color:#374151;">${escapeHtml(step.description)}</p>
          ${tip}
        </div>
      </div>
      ${images}
...
function buildPhoneSystemSectionHtml(section) {
  const steps = section.steps
    .map((step, index) => buildPhoneSystemStepHtml(step, index))
    .join("");
  return `
    <h2 style="margin:0 0 8px;font-size:18px;color:#111827 !important;-webkit-text-fill-color:#111827;">${escapeHtml(section.number)}. ${escapeHtml(section.title)}</h2>
    ${
      section.intro
        ? `<p style="margin:0 0 14px;font-size:12px;line-height:1.7;color:#4b5563 !important;-webkit-text-fill-color:#4b5563;">${escapeHtml(section.intro)}</p>`
        : ""
    }
    ${steps}
  `;
...
export async function exportPhoneSystemGuidePdf({ title, intro, sections = [] }) {
  const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4", compress: true });
  const blocks = [
    `
      <h1 style="margin:0 0 12px;font-size:22px;color:#111827 !important;-webkit-text-fill-color:#111827;">${escapeHtml(title)}</h1>
      <div style="margin:0 0 20px;padding:14px;background:#eef2ff;border:1px solid #c7d2fe;border-radius:12px;">
        <h2 style="margin:0 0 6px;font-size:15px;color:#111827 !important;-webkit-text-fill-color:#111827;">חיבור באמצעות SSO בלבד</h2>
        <p style="margin:0;font-size:13px;line-height:1.7;color:#374151 !important;-webkit-text-fill-color:#374151;">${escapeHtml(intro)}</p>
      </div>
    `,
    ...sections.map(buildPhoneSystemSectionHtml),
  ];

  for (let index = 0; index < blocks.length; index += 1) {
    const { root, cleanup } = await mountIsolatedGuideRoot(blocks[index]);
    try {
      const canvas = await captureGuideCanvas(root);
      addCanvasPagesToPdf(doc, canvas, index > 0);
    } finally {
      cleanup();
    }
  }

  doc.save("phone-system-guide.pdf");
}
