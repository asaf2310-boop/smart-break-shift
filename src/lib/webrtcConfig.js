...
/** Metered TURN — several transports improve firewall traversal */
...
export function getDefaultIceServers() {
  ...
  if (!servers) {
    servers = hasTurn && isMeteredTurnHost(turnUrl)
...
function getIceTransportPolicy() {
  ...
  if (turnUrl && turnUsername && turnCredential) return "relay";
  if (raw) {
    try {
      const parsed = JSON.parse(raw);
      if (
