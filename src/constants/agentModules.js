/** מזהה ישן — מעניק גישה למדריך תשלומים (תאימות לאחור) */
...
export function agentHasModule(modules, moduleId) {
  ...
  if (hasLegacyKnowledgeModule(modules) && moduleId === "knowledge_guide") {
    return true;
  }
...
export function pathnameAllowedByModules(pathname, modules) {
  ...
  for (const moduleId of AGENT_MODULE_IDS) {
    if (moduleId === "knowledge_guide") continue;
