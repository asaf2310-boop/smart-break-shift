...
function AgentGroup({ group, blobKeys, onPlayRecording }) {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center justify-between gap-3 px-4 py-3 bg-gradient-to-l from-slate-50 to-white hover:bg-slate-50 transition-colors text-right"
      >
  ...
  return (
    <div className="space-y-4" dir="rtl">
      ...
      <div className="flex flex-wrap gap-3 text-xs text-slate-600">
        <span>
          <strong className="text-slate-800">{groups.length}</strong> נציגים
        </span>
