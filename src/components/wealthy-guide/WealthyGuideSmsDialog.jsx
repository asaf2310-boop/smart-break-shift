...
export default function WealthyGuideSmsDialog({ open, onOpenChange, initialPhone = "" }) {
  ...
  const preview = useMemo(() => buildWealthyGuideSmsPreview(variant), [variant]);
  ...
  const handleSend = async (event) => {
    ...
    setSending(true);
    try {
      const result = await sendWealthyGuideLinksSms({ phone: normalizedPhone, variant });
      if (!result.ok) {
        toast({
          title: "לא נשלח",
