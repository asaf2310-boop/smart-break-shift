...
export function LoginShell({
  ...
  const heroLayout = demoHero || brandHero;

  return (
    <motion.div
      className={cn(
        "login-shell",
        heroLayout && "login-hero-shell login-hero-bg",
        demoHero && "login-shell--demo",
        brandHero && "login-shell--brand",
        production && !heroLayout && "login-shell--prod m3-page"
      )}
      dir="rtl"
    >
      {!heroLayout && (
      ...
      <motion.div
        className={cn("login-shell__content", heroLayout && "login-shell__content--demo")}
        initial={
          heroLayout &&
        ...
        transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1], delay: heroLayout ? 0.08 : 0 }}
      >
        <motion.div
          initial={
            heroLayout || production
              ? false
              : typeof window !== "undefined" &&
                  window.matchMedia("(prefers-reduced-motion: reduce)").matches
                ? false
          ...
          animate={production && !heroLayout ? { opacity: 1 } : { opacity: 1, scale: 1, y: 0 }}
          transition={{ duration: heroLayout || production ? 0 : 0.35, ease: [0.22, 1, 0.36, 1] }}
          className={cn(
            "relative z-10 w-full min-h-0",
            heroLayout ? "login-shell__card-wrap" : "max-w-sm sm:max-w-md px-2"
          )}
        >
          <div
            className={cn(
              "login-shell__card w-full",
              heroLayout ? "login-shell__card--demo login-card" : "p-5 sm:p-8"
            )}
          >
