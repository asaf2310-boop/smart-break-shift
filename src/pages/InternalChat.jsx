...
export default function InternalChat() {
  ...
  const [agentName] = useState(() => getStoredAgentName());
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50 to-purple-50" dir="rtl">
      <div className="relative z-10 max-w-6xl mx-auto px-3 sm:px-4 py-5 sm:py-8">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white shadow-lg overflow-hidden">
          <div className="px-4 py-4 sm:px-6 border-b border-slate-100 flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg sm:text-xl font-extrabold text-slate-800">צ'אט פנימי</h1>
              <p className="text-xs text-slate-500">שיחה כללית + הודעות אישיות בין נציגים</p>
              {localChat && (
                <div className="mt-1 inline-flex px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 text-[11px] font-bold border border-emerald-200">
          ...
          <div className="grid lg:grid-cols-[280px_1fr] min-h-[65vh]">
            <aside className="border-b lg:border-b-0 lg:border-l border-slate-100 p-4 space-y-4">
                ...
                <div className="space-y-2">
                  {privatePeers.map((peer) => (
                    <button
                      ...
                      className={`w-full text-right px-3 py-2 rounded-xl border text-sm font-semibold ${
                        selectedPeer === peer
              ...
              <div className="pt-2 border-t border-slate-100">
                <h2 className="text-xs font-bold text-slate-500 mb-2">סטטוס נציגים</h2>
                  ...
                  {AGENT_NAMES.map((agent) => {
                    ...
                    return (
                      <div key={agent} className="flex items-center justify-between gap-2">
                        ...
                        <span className={`text-xs px-2 py-1 rounded-full border ${statusClass(status.tone)}`}>
                          {status.label}
                        </span>
                      </div>
                    );
            ...
            <section className="p-4 sm:p-5 flex flex-col">
              <div className="flex-1 rounded-2xl border border-slate-100 bg-slate-50 p-3 sm:p-4 overflow-y-auto space-y-2">
                  ...
                  visibleMessages.map((msg) => {
                    ...
                    return (
                      <div key={msg.id} className={`flex ${mine ? "justify-start" : "justify-end"}`}>
                        <div className={`max-w-[80%] rounded-2xl px-3 py-2 ${mine ? "bg-indigo-500 text-white" : "bg-white border border-slate-200 text-slate-700"}`}>
